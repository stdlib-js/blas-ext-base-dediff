...
var dediff = tryRequire( resolve( __dirname, './../lib/dediff.native.js' ) );
var opts = {
	'skip': ( dediff instanceof Error )
};
...
tape( 'the function has an arity of 11', opts, function test( t ) {
	t.strictEqual( dediff.length, 11, 'returns expected value' );
	t.end();
});
...
tape( 'the function calculate differences between consecutive elements of a double-precision floating-point strided array', opts, function test( t ) {
	...
	out = dediff( x.length, x, 1, 2, p, 1, 2, a, 1, o, 1 );
...
tape( 'if the sum of the `N`, `N1`, and `N2` parameters is less than or equal to `1`, the function returns `out` unchanged', opts, function test( t ) {
	var expected;
	var out;
	var x;
	var p;
	...
	out = dediff( 1, x, 1, 0, p, 1, 0, a, 1, o, 1 );
	expected = new Float64Array([
		0.0,
		0.0,
		0.0,
...
tape( 'the function supports stride parameters', opts, function test( t ) {
	...
	out = dediff( 3, x, 2, 2, p, 2, 2, a, 2, o, 2 );
	expected = new Float64Array([
		10.0,
		0.0,
		25.0,
...
tape( 'the function supports no prepend and no append', opts, function test( t ) {
	...
	out = dediff( x.length, x, 1, 0, p, 1, 0, a, 1, o, 1 );
...
tape( 'the function supports multiple prepend and append values', opts, function test( t ) {
	...
	out = dediff( x.length, x, 1, 2, p, 1, 0, a, 1, o, 1 );
	...
	out = dediff( x.length, x, 1, 1, p, 1, 2, a, 1, o, 1 );
...
tape( 'the function supports append without prepend', opts, function test( t ) {
	...
	out = dediff( x.length, x, 1, 0, p, 1, 1, a, 1, o, 1 );
